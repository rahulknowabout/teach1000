...
/// show_user functions   
 function show_users() {
 if( isset($_REQUEST['search']) && $_REQUEST['search'] != "" ) { 
	$where = "where user_profile_name like '%".$_REQUEST['search']."%' or user_email like '%".$_REQUEST['search']."%'";
//	echo $where;
	}
  ...
  if( count( $rowc ) > 0 ) { 	
	...
	foreach($rowe as $row){
?>	    
		<tr><td><?php echo $count;   ?> </td>  <td>  <img src="uploadimages/<?php  if ( $row['user_p_photo'] != "" ) { echo $row['user_p_photo'];  } else{ echo "adminteach1000.jpg";} ?>" width="50px" height="40px" />  </td> 
        
		<td><?php echo $row['user_email'] ;  ?>  </td>  <td><?php echo $row['user_profile_name'] ;  ?>  </td><td><a href="index.php?f=user&t=users&bid=<?php echo $row['status']; ?>&ubid=<?php echo $row['user_id']        ; ?>&act=act"  class="btn btn-warning"<i class="fa fa-pencil"></i> <?php if($row['status'] == 0){ echo "Block"; } else { echo "unblock"; }?></a></td>
        <td> <a href="index.php?f=user&t=useraddedit&uid=<?php echo $row['user_id']; ?>&edi=edi"  class="btn btn-info btn-xs"><i class="fa fa-pencil"></i>View/Edit</a></td>
        <td> <a href="index.php?f=user&t=users&uid=<?php echo $row['user_id']; ?>&del=del&act=act" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i>DELETE</a></td>
         </tr>	
<?php 
		 $count++;
}
